...
class Debug_Bar {
	...
	function render() {
		...
		$panels = array();
		$index = 0;
		foreach ( $this->panels as $panelObj ) {
			$panel = array();
			$identifier = spl_object_hash($panelObj);
			...
			$panel['link'] = '<li><a
				id="debug-menu-link-'. $identifier .'"
				class="debug-menu-link'. (($index === 0) ? ' current' : '') .'"
				href="#debug-menu-target-'. $identifier .'">'. $panelObj->title() .'</a></li>';

			$panel['content'] = '<div id="debug-menu-target-'. $identifier .'" class="debug-menu-target"'. (($index === 0) ? ' style="display:block"' : '') .'">' . $content .'</div>';
			$panels[] = $panel;
			$index++;
		}
